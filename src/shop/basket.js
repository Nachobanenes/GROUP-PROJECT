...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    cartItems.forEach(itemContainer => {
        const isChecked = itemContainer.querySelector('.cart-item-checkbox').checked;
        if (!isChecked) return;
    
        const id = itemContainer.id.replace('b', '');
        const qty = Number(document.getElementById(`v${id}`).value);
        const finalPrice = parseFloat(itemContainer.querySelector('.price-current').innerText.replace('₱', ''));
        const originalPriceElement = itemContainer.querySelector('.price-original');
